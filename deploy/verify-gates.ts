...
const check = (condition: boolean, msg: string) => { if (!condition) { console.error(`❌ ${msg}`); process.exit(1); } console.log(`✅ ${msg}`); };
...
check(cfg.escapeHatch && cfg.escapeHatch.testedOnTestnet === true, 'escape hatch tested on testnet');
check(cfg.escapeHatch && cfg.escapeHatch.evidenceUrl && cfg.escapeHatch.evidenceUrl.startsWith('https://'), 'escape hatch evidence URL is HTTPS');
