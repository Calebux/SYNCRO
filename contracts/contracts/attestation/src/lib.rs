...
impl AttestationContract {
    ...
    pub fn init(env: Env, issuer: Address) -> Result<(), AttestError> {
        if env.storage().instance().has(&Key::Issuer) {
            return Err(AttestError::AlreadyInit);
        }
    ...
    pub fn issue(
        ...
        if digest == BytesN::from_array(&env, &[0u8; 32]) {
            return Err(AttestError::InvalidDigest);
        }
        ...
        env.storage().persistent().set(
            &Key::Record(subject.clone(), attestation_type.clone()),
            &record,
        );
        Issued {
            subject,
    ...
    pub fn verify(
        ...
        match env
            .storage()
            .persistent()
            .get::<Key, AttestRecord>(&Key::Record(subject, attestation_type))
        {
    ...
    pub fn get_record(
        ...
        env.storage()
            .persistent()
            .get(&Key::Record(subject, attestation_type))
    }
    ...
    fn require_issuer(env: &Env) -> Result<(), AttestError> {
        let issuer: Address = env
            ...
            .instance()
            .get(&Key::Issuer)
            .ok_or(AttestError::Unauthorized)?;
        issuer.require_auth();
        Ok(())
    }
}
