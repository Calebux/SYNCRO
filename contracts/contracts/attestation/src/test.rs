...
mod tests {
    ...
    #[test]
    fn test_revoke_nonexistent_errors() {
        let (env, client, _) = setup();
        let subject = Address::generate(&env);
        let atype = Symbol::new(&env, "KYC_BASIC");
    ...
    fn test_zero_digest_rejected() {
        ...
        assert_eq!(
            client.issue(&subject, &atype, &zero),
    ...
    #[test]
    fn test_multiple_attestation_types_independent() {
        let (env, client, _) = setup();
        let subject = Address::generate(&env);
